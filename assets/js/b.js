...
var LiquidButton = function () {
  function LiquidButton(optionsParam) {
    ...
    this.width = options.width || 165;
    this.height = options.height || 57;
    ...
    this.color1 = options.color1 || '#2c343e';
    this.color2 = options.color2 || '#14a0ff';
    this.color3 = options.color3 || '#0073c2';
    this.textColor = options.textColor || 'rgba(255, 255, 255, .9)';
    this.layers = [{
      points: [],
      viscosity: 0.5,
      mouseForce: 100,
    ...
    this.getBonus  = document.getElementById('get-bonus');
    this.bonusData = document.getElementById('bonus-data');
    this.canvas = options.canvas || document.getElementById('get-bonus');
    ...
    function t () {
      ...
      if ( wW <= 480 && !isLandscape || wW <= 800 && isLandscape )
      {
        smallSize();
      } else {
        bigSize();
      ...
      if ( self.canvas )
      {
        self.canvas.outerHTML = "";
      }
        var getBonus = document.createElement('canvas');
        self.bonusData.parentNode.insertBefore(getBonus, self.bonusData.nextSibling);
        self.canvas = getBonus;
        self.text = self.bonusData.getAttribute('data-title');
        self.context = self.canvas.getContext('2d');

        self.canvas.setAttribute('id', 'get-bonus');
    }
    ...
    this.canvas.addEventListener('click', gotoHref, false);
  ...
  LiquidButton.prototype.draw = function draw() {
    ...
    this.context.font = '400 ' + this.fontSize + 'px "Source Sans Pro"';
